...
export const LimitedTextInput = memo(
  ({ text, setText, maxLength, placeHolderContent }: LimitedTextInputProps) => {
...
export const RegularTextInput = memo(
  ({ text, setText, placeHolderContent }: TextInputProps) => {
    ...
    return (
      <div className="border-grey03 flex h-fit w-full items-center justify-center gap-2.5 rounded-sm border px-3.5 py-2.5">
        <textarea
          ref={textareaRef}
          value={text}
          onChange={(e) => setText(e.target.value)}
